...
# Resolve photos path to absolute
$PhotosAbs = (Resolve-Path -Path $Photos -ErrorAction SilentlyContinue)?.Path
if (-not $PhotosAbs) {
...
# ── Build ──────────────────────────────────────────────────────────────────
$needsBuild = $Rebuild -or -not (docker image inspect $ImageName 2>$null | Select-String $ImageName)
if ($needsBuild) {
...
$DisplayHost = ""
if (-not $Headless) {
    # Docker Desktop on Windows routes host.docker.internal → your machine
